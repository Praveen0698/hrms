...
const GrantLoanForm = () => {
  const {
    formData, setFormData, permittedByError, setPermittedByError, loanDetailsByError, setLoanDetailsByError,loan,setLoan,formvisible,setFormVisible,
    pay, setPay, emiPay, setEmiPay, emiClear, setEmiClear
  } = GrantLoanState();
  ...
  const saveLoan = async () => {
    ...
    alert("GrantLoan form added successfully");
    navigate("/loan");
    loadLoan();
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'permittedBy') {
      if (!isRequired) {
        ...
        setPermittedByError('Permitted By should only contain alphabetic characters');
      } else if (!isValidSize) {
        setPermittedByError('Permitted By should be between 2 and 40 characters');
      } else {
        setPermittedByError('');
      }
      ...
      if (!isRequired) {
        ...
        setLoanDetailsByError('Loan Details should be between 2 and 40 characters');
      } else {
  ...
  const repaymentAmtTotal = () => {
    var total = parseFloat(formData.amount) + (parseFloat(formData.amount) * (parseFloat(formData.interestPersentage) / 100));
  ...
  useEffect(() => {
    ...
    emi();
    cleared();
  }, [formData.amount, formData.interestPersentage, formData.installmentPeriod, formData.installmentCleared]);
