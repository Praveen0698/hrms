...
const PerformanceIndicatorView = () => {
  ...
  const savePerformance = async (e) => {
    e.preventDefault();
    await axios.post(
      "http://localhost:8083/performance/create/performance",
      formData
    );
  ...
  const loadPerformance = async () => {
    const result = await axios.get(
      "http://localhost:8083/performance/get/performance",
      {
        validateStatus: () => {
          return true;
        },
  ...
  const handleDelete = async (id) => {
    console.log(id);
    await axios.delete(`http://localhost:8083/performance/delete/${id}`);
    loadPerformance();
  };
